...
impl Config {
    ...
    fn load_chains() -> Result<Vec<ChainConfig>> {
        ...
        // Ethereum Mainnet
        if let (Ok(ws), Ok(http)) = (env::var("ETH_MAINNET_WS"), env::var("ETH_MAINNET_HTTP")) {
        ...
        // Arbitrum
        if let (Ok(ws), Ok(http)) = (env::var("ARBITRUM_WS"), env::var("ARBITRUM_HTTP")) {
            chains.push(ChainConfig {
                chain_id: 42161,
        ...
        // Optimism
        if let (Ok(ws), Ok(http)) = (env::var("OPTIMISM_WS"), env::var("OPTIMISM_HTTP")) {
            chains.push(ChainConfig {
                chain_id: 10,
        ...
        // Base
        if let (Ok(ws), Ok(http)) = (env::var("BASE_WS"), env::var("BASE_HTTP")) {
            chains.push(ChainConfig {
                chain_id: 8453,
                name: "Base".to_string(),
                rpc_ws: ws,
        ...
        if chains.is_empty() {
            anyhow::bail!(
                "No blockchain chains configured. Set at least one of: ETH_MAINNET_WS, ARBITRUM_WS, OPTIMISM_WS, BASE_WS"
            );
        }
    ...
    fn load_observability() -> Result<ObservabilityConfig> {
        Ok(ObservabilityConfig {
            rust_log: env::var("RUST_LOG").unwrap_or_else(|_| "info,ethhook=debug".to_string()),
