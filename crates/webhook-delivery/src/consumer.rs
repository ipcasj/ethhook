...
impl JobConsumer {
    ...
    pub async fn new(redis_url: &str, queue_name: &str) -> Result<Self> {
        info!("Connecting to Redis at {} (queue: {})", redis_url, queue_name);
        ...
        match result {
            Some((_queue, job_json)) => {
                ...
                let job: DeliveryJob = serde_json::from_str(&job_json)
                    .context("Failed to parse delivery job JSON")?;
                
                debug!(
                    "Consumed job: endpoint={} attempt={} url={}",
                    job.endpoint_id, job.attempt, &job.url[..30.min(job.url.len())]
                );
    ...
    pub async fn requeue(&mut self, mut job: DeliveryJob) -> Result<()> {
        ...
        let job_json = serde_json::to_string(&job)
            .context("Failed to serialize job for requeue")?;
