...
fn main() {
    ...
    validate_rpc_endpoint(&mut errors, &mut warnings, "ETH_RPC_WS", "Ethereum WebSocket (primary)");
    validate_rpc_endpoint(&mut errors, &mut warnings, "ETH_RPC_HTTP", "Ethereum HTTP (primary)");
    validate_optional_rpc(&mut warnings, "ETH_RPC_WS_BACKUP", "Ethereum WebSocket (backup)");
    validate_optional_rpc(&mut warnings, "ETH_RPC_HTTP_BACKUP", "Ethereum HTTP (backup)");
    
    validate_rpc_endpoint(&mut errors, &mut warnings, "ARBITRUM_RPC_WS", "Arbitrum WebSocket (primary)");
    validate_rpc_endpoint(&mut errors, &mut warnings, "ARBITRUM_RPC_HTTP", "Arbitrum HTTP (primary)");
    validate_optional_rpc(&mut warnings, "ARBITRUM_RPC_WS_BACKUP", "Arbitrum WebSocket (backup)");
    validate_optional_rpc(&mut warnings, "ARBITRUM_RPC_HTTP_BACKUP", "Arbitrum HTTP (backup)");
    
    validate_rpc_endpoint(&mut errors, &mut warnings, "OPTIMISM_RPC_WS", "Optimism WebSocket (primary)");
    validate_rpc_endpoint(&mut errors, &mut warnings, "OPTIMISM_RPC_HTTP", "Optimism HTTP (primary)");
    validate_optional_rpc(&mut warnings, "OPTIMISM_RPC_WS_BACKUP", "Optimism WebSocket (backup)");
    validate_optional_rpc(&mut warnings, "OPTIMISM_RPC_HTTP_BACKUP", "Optimism HTTP (backup)");
    
    validate_rpc_endpoint(&mut errors, &mut warnings, "BASE_RPC_WS", "Base WebSocket (primary)");
    validate_rpc_endpoint(&mut errors, &mut warnings, "BASE_RPC_HTTP", "Base HTTP (primary)");
    validate_optional_rpc(&mut warnings, "BASE_RPC_WS_BACKUP", "Base WebSocket (backup)");
    ...
    validate_numeric(&mut errors, "API_RATE_LIMIT_PER_MINUTE", 1, 10000, Some(100));
    println!();
    ...
    validate_optional(&mut warnings, "OTEL_EXPORTER_OTLP_ENDPOINT", "http://localhost:4317");
    validate_numeric(&mut errors, "PROMETHEUS_METRICS_PORT", 1024, 65535, Some(9090));
    println!();
    ...
    match env::var(key) {
        Ok(value) => {
            match value.parse::<u32>() {
                ...
                Ok(num) => {
                    errors.push(format!("{} value {} is out of range ({}-{})", key, num, min, max));
...
fn validate_rpc_endpoint(errors: &mut Vec<String>, warnings: &mut Vec<String>, key: &str, description: &str) {
    match env::var(key) {
        Ok(value) if value.starts_with("wss://") || value.starts_with("https://") => {
            if value.contains("YOUR_KEY") || value.contains("YOUR_PROJECT_ID") {
                ...
                println!("  ⚠ {}: {} (contains placeholder)", description, mask_url(&value));
            } else {
                println!("  ✓ {}: {}", description, mask_url(&value));
            }
        }
        Ok(value) => {
            errors.push(format!("{} must start with wss:// or https://, got: {}", key, value));
...
fn validate_redis(errors: &mut Vec<String>, key: &str) {
    match env::var(key) {
        ...
        Ok(value) => {
            errors.push(format!("{} must start with redis:// or rediss://, got: {}", key, value));
...
fn validate_jwt_secret(errors: &mut Vec<String>) {
    match env::var("JWT_SECRET") {
        Ok(value) if value.len() >= 32 => {
            println!("  ✓ JWT_SECRET: {} (length: {})", mask_secret(&value), value.len());
        }
        Ok(value) => {
            errors.push(format!("JWT_SECRET must be at least 32 characters, got {}", value.len()));
            println!("  ✗ JWT_SECRET: too short ({} chars, need 32+)", value.len());
        }
        Err(_) => {
...
fn mask_url(url: &str) -> String {
    ...
    if let Some(pos) = url.rfind("/v2/").or_else(|| url.rfind("/v3/")) {
        ...
        if suffix.len() > 10 {
            format!("{}{}...{}", prefix, &suffix[..5], &suffix[suffix.len()-3..])
        } else {
            format!("{}***", prefix)
        }
    } else {
