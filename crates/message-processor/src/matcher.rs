...
impl EndpointMatcher {
    ...
    /// # Arguments
    ///
    /// * `pool` - SQLite connection pool from ethhook_common::create_pool
    ...
    pub async fn find_matching_endpoints(
        ...
        // 3. (endpoint.event_topics matches event.topics OR endpoint.event_topics IS NULL)
        //    Note: SQLite uses JSON or text array comparison
        //
        // Example:
...
mod tests {
    ...
    async fn test_matcher_creation() {
        let pool = sqlx::SqlitePool::connect("postgresql://localhost/ethhook_test")
            .await
            .unwrap();
