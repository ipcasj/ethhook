...
impl DeliveryPublisher {
    ...
    pub async fn new(redis_url: &str, queue_name: &str) -> Result<Self> {
        info!("Connecting to Redis for delivery publishing at {}", redis_url);
    ...
    pub async fn publish(
        ...
        let job_json = serde_json::to_string(&job)
            .context("Failed to serialize delivery job")?;
...
mod tests {
    ...
    async fn test_publisher_creation() {
        let publisher = DeliveryPublisher::new(
    ...
    async fn test_publish_job() {
        let mut publisher = DeliveryPublisher::new(
            "redis://localhost:6379",
            "test_delivery_queue",
        )
        .await
