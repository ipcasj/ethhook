...
async fn test_deduplication_stores_event_ids() {
    ...
    let mut dedup = Deduplicator::new("redis://127.0.0.1:6379", 3600)
        .await
        .unwrap(); // 1 hour TTL
...
async fn test_stream_publishing() {
    ...
    let mut publisher = StreamPublisher::new("redis://127.0.0.1:6379")
        .await
        .unwrap();
    ...
    assert!(
        stream_id.contains('-'),
        "Stream ID should contain timestamp-sequence"
    );
...
async fn test_multiple_chains_separate_streams() {
    ...
    let mut conn = client.get_multiplexed_async_connection().await.unwrap();
    let _: () = conn
        .del(&["eth:events:1", "eth:events:42161"])
        .await
        .unwrap();
}
...
async fn test_event_id_uniqueness() {
    ...
    assert_ne!(
        id1, id3,
        "Different block_number should create different IDs"
    );
